...
double CalculaAreaTriangulo(Triangulo t) {
	double area=abs(t.v1.northing*t.v2.easting + t.v2.northing*t.v3.easting + t.v3.northing*t.v1.easting-t.v1.easting*t.v2.northing - t.v2.easting*t.v3.northing - t.v3.easting*t.v1.northing)*0.5;
	return area;
}
...
bool isIn(coord pdado){
    ...
    for(i=0; i<gNumVertPolig-1;i++ ){
        n = CriaTriangulo(coordenadas[i], coordenadas[i+1], pdado);
        areaTotal = CalculaAreaTriangulo(n) + areaTotal;
    }
    if (areaTotal == CalcularAreaPoligono(coordenadas,gNumVertPoli))
		return true;
	return false;
}
                ...
                if (abriu)  {
                    ...
                    switch (opcaoMenu) {
                        ...
                        case: 5
                         ...
                         case 6:
                          double UTM2LatLon;
                          UTM2LatLon = convertUTM2LatLon(coordenadas,zona,hemisferio); 
